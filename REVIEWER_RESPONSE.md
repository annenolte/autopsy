...
| 14 | No cost/time per KLOC/file/vuln; no scaling experiment | ✅ Mostly | `TIMING.md`: time/KLOC + time/vuln measured (~273 vs ~275 s/KLOC — flat across 8× size). **Token accounting added** to the client (`get_usage`), harness reports tokens + tokens/KLOC → dollar cost = tokens × price. Token numbers populate on the (staged) live run. |
