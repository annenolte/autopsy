...
| 6 | Deletion / comment-activation is core novelty but **no benchmark vuln tests it** | ❌ Code/experiment TODO | Add deletion-based / comment-activation cases to the benchmark and measure the zero-footprint detector. (Detector exists in code; untested in eval.) **High priority — reviewer cares.** |
...
| 18 | Run Semgrep/CodeQL on the same benchmark; fix inaccurate capability claims | 🟡 Partial | "Single Sonnet, no graph" baseline ✅. **Still:** actually run Semgrep + CodeQL on the benchmark (commands are in benchmark/pygoat/README), and correct the Semgrep cross-file / "file-level tools can't detect this" claims. |
