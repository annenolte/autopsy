...
| 2 | Tiny benchmark; use an established benchmark + frozen repo + **pre-registered matching protocol fixed before scoring** | 🟡 Mostly | Now on **TWO external benchmarks**: pygoat (real app) + the named **SecurityEval** dataset (121 CWE files; `SECURITYEVAL.md`). Token-free baselines run (Semgrep 19%, Bandit 40%, Autopsy-deterministic 3%); **Autopsy LLM run on SecurityEval staged (needs tokens)**. Protocol frozen (`FROZEN_PROTOCOL.md`). **Still:** the Autopsy LLM number on SecurityEval; the other named sets (Juliet=mostly C/Java, PrimeVul/ReposVul = large lift). |
| 3 | Multi-language claim but only Python/Flask tested | 🟡 Partial | `benchmark/js_demo/` adds a cross-file **TypeScript** benchmark (8 vulns). Surfaced + **fixed** a real bug (exported TS functions weren't extracted → empty graph). Honest finding documented: even fixed, anonymous Express handlers + cross-file call edges aren't captured, so the graph mechanism is Python-centric and only partial for idiomatic JS/TS — **scope the claim**. Semgrep baseline 4/8. **Still:** the LLM scan on TS (needs tokens). |
...
| 8 | 7-signal authorship heuristic: hand weights, anecdotal validation, fires on human code | 🟡 Partial | `benchmark/eval_authorship.py` + `AUTHORSHIP.md`: measured FP rate on **1,272 human (pre-2021) files** from requests+flask — ~2% as-shipped / ~7% code-only (not the rampant over-firing assumed). Surfaced a confidence-math quirk. **Still:** labeled AI-positive set for full P/R/AUC; retrain weights. |
| 9 | Is benchmark code actually AI-generated? Authorship detector never validated | ✅ Done (artifact) | Validated on the **SecurityEval** labeled set. Split result: (a) **explicit AI markers now reliably detected** after fixing a real aggregation bug (a Co-Authored-By:Claude trailer was diluted below threshold; now decisive, human FP unchanged ~2%); (b) **unmarked** AI code still not separable (ROC-AUC 0.42, per-signal AUC 0.41–0.52). Paper framing: marker-detection + soft prioritization, **not** a general unmarked-AI classifier; report vuln-detection separately. (`eval_authorship_classifier.py`, AUTHORSHIP.md) |
...
| 13 | Only overall TP/FP/F1; no per-category | ✅ Done (artifact) | `benchmark/per_category.py` re-scores a saved run and prints per-category recall (offline). On pygoat: every category caught (SQLi 2/2, Code Injection 2/2, Insecure Deser 2/2, others 1/1). Put the table in the paper. |
| 14 | No cost/time per KLOC/file/vuln; no scaling experiment | 🟡 Partial | `benchmark/TIMING.md`: harness now records time/KLOC + time/vuln; measured ~273 s/KLOC (demo) vs ~275 s/KLOC (pygoat, ~8× larger) — roughly flat. **Still:** dollar cost/KLOC needs token accounting in the client (not captured); CodeQL not run (heavy CLI, documented in compare_tools.py). |
| 15 | depth-5 / 50-node subgraph cap not empirically tested | ✅ Done (artifact) | `benchmark/eval_subgraph_caps.py` + `SUBGRAPH_CAPS.md` measure it on 5 repos: median node is fine, but 5–13% of neighborhoods exceed the node cap and 4–9% of chains exceed 5 hops (max 8) on mature libraries — caps truncate a real minority. Quantified honestly; write into paper. Mitigation (adaptive caps for hubs) is future work. |
| 16 | 5 runs, only final reported; no variance / prompt / model / Haiku-vs-Sonnet sensitivity | 🟡 Mostly | `--repeat` gives mean±std (variance ✅). **Sonnet-only-vs-Haiku+Sonnet ablation now built**: `scan_stream(use_triage=False)` + `--arm sonnet-only` / `--arm all` (autopsy vs sonnet-only vs raw). Token-free to build; **the comparison run is staged (needs tokens)**. Still: prompt-sensitivity and model-version sensitivity (not built). |
